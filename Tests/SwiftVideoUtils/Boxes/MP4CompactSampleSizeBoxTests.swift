...
final class MP4CompactSampleSizeBoxTests: XCTestCase {
    ...
    func test16BitReadWrite() async throws {
        let box = MP4CompactSampleSizeBox(version: .isoMp4(0), flags: .init(), fieldSize: 16, sampleSizes: [0, UInt16(UInt8.max), UInt16.max-1, UInt16.max])
    ...
    func test8BitReadWrite() async throws {
        let box = MP4CompactSampleSizeBox(version: .isoMp4(0), flags: .init(), fieldSize: 8, sampleSizes: [0, UInt16(UInt8.max)])
    ...
    func test4BitReadWrite() async throws {
        let boxEvenSamples = MP4CompactSampleSizeBox(version: .isoMp4(0), flags: .init(), fieldSize: 4, sampleSizes: [0, 13, 14, 15])
        ...
        let boxOddSamples = MP4CompactSampleSizeBox(version: .isoMp4(0), flags: .init(), fieldSize: 4, sampleSizes: [0, 14, 15])
